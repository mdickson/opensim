...
namespace Gloebit.GloebitMoneyModule
{
    ...
    public class GloebitAPI
    {
        ...
        private void PopulateTransactStageAndFailure(out TransactionStage stage, out TransactionFailure failure, bool success, string status, string reason)
        {
            ...
            {
                if (reason == "success")
                {                          /* successfully queued, early enacted all non-asset transaction parts */
                    switch (reason)
                    {
                        case "success":
            ...
            {                                /* successfully queued.  an early enact failed */
                ...
                {                     /* permanent failure - actionable by buyer */
                    failure = TransactionFailure.INSUFFICIENT_FUNDS;
                }
                else if (reason == "pending")
                {                           /* queue will retry enacts */
                ...
                {                                                    /* perm failure - assumes tp will get same response form part.enact */
                    // Shouldn't ever get here.
                    failure = TransactionFailure.ENACTING_GLOEBIT_FAILED;
                }
            }
            else
            {                                                        /* failure prior to successful queing.  Something requires fixing */
                if (reason == "unknown OAuth2 token")
                {                     /* Invalid Token.  May have been revoked by user or expired */
                    stage = TransactionStage.AUTHENTICATE;
                    failure = TransactionFailure.AUTHENTICATION_FAILED;
                }
                else if (status == "queuing-failed")
                {                    /* failed to queue.  net or processor error */
                    stage = TransactionStage.QUEUE;
                    failure = TransactionFailure.QUEUEING_FAILED;
                }
                else if (status == "failed")
                {                            /* race condition - already queued */
                    // nothing to tell user.  buyer doesn't need to know it was double submitted
                    stage = TransactionStage.QUEUE;
                    failure = TransactionFailure.RACE_CONDITION;
                }
                else if (status == "cannot-spend")
                {                      /* Buyer's gloebit account is locked and not allowed to spend gloebits */
                    stage = TransactionStage.VALIDATE;
                    failure = TransactionFailure.PAYER_ACCOUNT_LOCKED;
                }
                else if (status == "cannot-receive")
                {                    /* Seller's gloebit account can not receive gloebits */
                    // TODO: Check role in new system.  This is for role=Payee, not role=Application
                    stage = TransactionStage.VALIDATE;
                    failure = TransactionFailure.PAYEE_CANNOT_RECEIVE;
                }
                else if (status == "unknown-merchant")
                {                  /* can not identify merchant from params supplied by app */
                    stage = TransactionStage.VALIDATE;
                ...
                {                                                    /* App issue --- Something needs fixing by app */
                    stage = TransactionStage.VALIDATE;
